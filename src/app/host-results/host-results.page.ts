...
export class HostResultsPage implements OnInit {
  ...
  constructor(public navExtrasService: NavExtrasService, public api: ApiService, public loadingController: LoadingController, public alertController: AlertController, public storage: StorageService) {
    ...
    this.api.getHostDetails(item.ip_str).then((res) => {
      // SECURITY: Removed console.log that was leaking host details in plain text
      this.item = res;
  ...
  showPayload(port) {
    // SECURITY: Removed console.log that was leaking port payload in plain text
    const found = this.item.data.find(d => d.port === port);
    if (found) {
      // SECURITY: Removed console.log that was leaking found port in plain text
      this.selectedPort = found;
      // SECURITY: Removed console.log that was leaking selected port in plain text
    }
  }
