...
export class HostResultsPage implements OnInit {
  ...
  showPayload(port) {
    // SECURITY: Removed console.log that was leaking port details
    const found = this.item.data.find(d => d.port === port);
    if (found) {
      // SECURITY: Removed console.log that was leaking found port details
      this.selectedPort = found;
      // SECURITY: Removed console.log that was leaking selected port details
    }
  }
