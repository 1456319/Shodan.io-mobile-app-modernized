...
export class HomePage {
  ...
  getQueries() {
    this.api.getQueries().then((res) => {
      // SECURITY: Removed console.log that was leaking search query matches
      this.queries = res['matches']
    });
  }

  changeQuery(item) {
    // SECURITY: Removed console.log that was leaking the query item
  ...
  searchShodan(item: string) {
   this.storage.addSearch(item);
    // SECURITY: Removed console.log that was leaking the search item
