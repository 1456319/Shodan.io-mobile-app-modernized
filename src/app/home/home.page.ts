...
export class HomePage {
  ...
  getQueries() {
    this.api.getQueries().then((res) => {
      // SECURITY: Removed console.log that was leaking query search results in plain text
      this.queries = res['matches']
    });
  }

  changeQuery(item) {
    // SECURITY: Removed console.log that was leaking user query string in plain text
    this.query = item.query;
  }

  getMoreQueries(infiniteScroll) {
    this.api.getMoreQueries().then((res) => {
      // SECURITY: Removed console.log that was leaking more query search results in plain text
  ...
  searchShodan(item: string) {
   this.storage.addSearch(item);
    // SECURITY: Removed console.log that was leaking user search item in plain text
