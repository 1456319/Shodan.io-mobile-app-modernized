...
export class HomePage {
  ...
  getQueries() {
    this.api.getQueries().then((res) => {
      // SECURITY: Removed console.log that was leaking API queries
      this.queries = res['matches']
    });
  }

  changeQuery(item) {
    // SECURITY: Removed console.log that was leaking query
  ...
  searchShodan(item: string) {
   this.storage.addSearch(item);
    // SECURITY: Removed console.log that was leaking search item
