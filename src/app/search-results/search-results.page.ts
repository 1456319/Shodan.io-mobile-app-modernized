...
export class SearchResultsPage implements OnInit {
  ...
  constructor(public api: ApiService, public navExtrasService: NavExtrasService, public router: Router, public loadingController: LoadingController, public alertController: AlertController, public storage: StorageService) {
      ...
      // SECURITY: Removed console.log that was leaking the user's search query in plain text
  ...
  search(query) {
    this.api.search(query).then((res) => {
      // SECURITY: Removed console.log that was leaking raw search results in plain text
      this.results = res;
  ...
  getMoreResults(infiniteScroll) {
    this.api.getMoreResults(this.query).then((res) => {
      // SECURITY: Removed console.log that was leaking raw search result matches in plain text
      ...
      buttons: [
        {
          ...
          cssClass: 'secondary',
          handler: (blah) => {
            // SECURITY: Removed unnecessary debug console.log
          }
        }, {
          text: 'Yes!',
          handler: (data) => {
