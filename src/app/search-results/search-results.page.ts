...
export class SearchResultsPage implements OnInit {
  ...
  search(query) {
    this.api.search(query).then((res) => {
      // SECURITY: Removed console.log that was leaking the search results in plain text
      this.results = res;
  ...
  getMoreResults(infiniteScroll) {
    this.api.getMoreResults(this.query).then((res) => {
      // SECURITY: Removed console.log that was leaking the paginated search results in plain text
