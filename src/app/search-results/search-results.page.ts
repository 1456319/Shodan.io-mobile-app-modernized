...
export class SearchResultsPage implements OnInit {
  ...
  search(query) {
    this.api.search(query).then((res) => {
      // SECURITY: Removed console.log that was leaking search results
      this.results = res;
  ...
  getMoreResults(infiniteScroll) {
    this.api.getMoreResults(this.query).then((res) => {
      // SECURITY: Removed console.log that was leaking match results
      ...
      buttons: [
        {
          ...
          cssClass: 'secondary',
          handler: (blah) => {
            // SECURITY: Removed console.log that was leaking debug info
          }
        }, {
          text: 'Yes!',
          handler: (data) => {
