...
export class QueriesPage implements OnInit {
  ...
  getQueries() {
    this.api.getQueries().then((res) => {
      // SECURITY: Removed console.log that was leaking queries matches in plain text
  ...
  getMoreQueries(infiniteScroll) {
    this.api.getMoreQueries().then((res) => {
      // SECURITY: Removed console.log that was leaking more queries matches in plain text
