...
export function parseApiError(error: unknown): ShodanApiError {
  ...
  const sanitizeMessage = (msg?: string) => {
    if (!msg) return msg;
    return msg.replace(/key=[^&]+(&|$)/g, 'key=***$1');
  };
  ...
  if (e.response) {
    ...
    return new ShodanApiError(s, sanitizeMessage(e.message) ?? `Request failed with status ${s}`);
  }
  if (e.code === 'ECONNABORTED' || e.code === 'ETIMEDOUT') return new ShodanApiError(408, 'Request timed out.');
  return new ShodanApiError(0, sanitizeMessage(e.message) ?? 'Network error.');
}
