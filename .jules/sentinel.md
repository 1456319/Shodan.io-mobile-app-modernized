...
**Vulnerability:** The application was logging sensitive search queries and host details to the browser console in `src/app/queries/queries.page.ts`, `src/app/home/home.page.ts` and `src/app/host-results/host-results.page.ts`.
**Learning:** Developers often use `console.log` for debugging during development and forget to remove them before production, leading to unintentional information leakage of internal application state and user data.
**Prevention:** Establish a strict policy against logging sensitive data. Use a dedicated logging service that automatically strips or masks sensitive information before writing to logs.
