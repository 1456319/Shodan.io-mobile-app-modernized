...
**Vulnerability:** The application was logging sensitive user search queries, search history, and search results to the browser console in `src/app/search-results/search-results.page.ts` and `src/app/history/history.page.ts`.
**Learning:** Developers often use `console.log` for debugging search implementations and forget to remove them, leading to unintentional information leakage of user search behavior and sensitive query details.
**Prevention:** Establish a strict policy against logging sensitive data such as user search queries or history. Use safe logging abstractions that sanitize or omit sensitive user input before writing to logs.
