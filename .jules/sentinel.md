...
**Vulnerability:** The application was logging sensitive user search history to the console (`console.log(this.searches);`) in `src/app/history/history.page.ts`.
**Learning:** Developers often use `console.log` for debugging during development and forget to remove them before production, leading to unintentional information leakage of internal application state, in this case, a user's search history which may contain PII or sensitive queries.
**Prevention:** Establish a strict policy against logging sensitive user data, particularly search histories, to the console. Use a dedicated logging service that automatically strips or masks sensitive information before writing to logs or error tracking systems.
