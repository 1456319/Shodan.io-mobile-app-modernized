...
## 2026-06-24 - [CRITICAL] Fix Sensitive Data Leak in Console Logs
**Vulnerability:** The application was logging sensitive user search history and bookmarks to the console in `src/app/history/history.page.ts` and `src/app/my-searches/my-searches.page.ts`.
**Learning:** Even when debugging user preferences or history, logging user-specific data like searches and bookmarks can lead to significant information leakage of user activity.
**Prevention:** Strictly enforce a "no console.log" policy for production code, especially when dealing with data retrieved from user history or preferences.
