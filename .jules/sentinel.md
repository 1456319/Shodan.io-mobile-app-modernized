...
## 2026-06-15 - [CRITICAL] Fix Sensitive Data Leak in Console Logs
**Vulnerability:** The application was logging user search queries and raw search results to the console in `src/app/search-results/search-results.page.ts`.
**Learning:** Developers often use `console.log` for debugging during development and forget to remove them before production, leading to unintentional information leakage of user queries and application data.
**Prevention:** Establish a strict policy against logging sensitive data or generic data objects. Use a dedicated logging service that automatically strips or masks sensitive information before writing to logs or error tracking systems.
