...
## 2026-05-06 - [CRITICAL] Fix Sensitive Data Leak in Console Logs
**Vulnerability:** The application was logging sensitive user search history, bookmarks, and network alerts to the console in `src/app/history/history.page.ts`, `src/app/my-searches/my-searches.page.ts`, and `src/app/alerts/alerts.page.ts`.
**Learning:** Developers often use `console.log` for debugging during development and forget to remove them before production, leading to unintentional information leakage of internal application state and sensitive user data.
