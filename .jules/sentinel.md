...
## 2026-05-21 - [CRITICAL] Fix Sensitive Data Leak in Console Logs
**Vulnerability:** The application was logging sensitive user search history and bookmarks to the browser console (`console.log(this.searches);` and `console.log(this.bookmarks);`) in `src/app/history/history.page.ts` and `src/app/my-searches/my-searches.page.ts`.
**Learning:** Developers often use `console.log` for debugging during development and forget to remove them before production, leading to unintentional information leakage of user data like search history and bookmarks.
**Prevention:** Establish a strict policy against logging sensitive user data to the console. Review all console logs before committing and use automated linting rules to prevent `console.log` statements in production builds.
