...
**Vulnerability:** The application was logging sensitive user search queries and history data directly to the console (`console.log(this.searches);`, `console.log(this.query);`) in files like `src/app/history/history.page.ts` and `src/app/search-results/search-results.page.ts`.
**Learning:** Similar to other console.log leaks, developers often forget to remove debugging statements that expose user activities and inputs, which can be sensitive information.
**Prevention:** Establish a strict policy against logging user inputs, search queries, or internal application state. Use a dedicated logging service that strips sensitive information.
