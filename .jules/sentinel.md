...
## 2024-05-22 - [CRITICAL] Fix Sensitive Data Leak in Console Logs
**Vulnerability:** The application was logging the user's entire search history to the browser console (`console.log(this.searches);`) in `src/app/history/history.page.ts`.
**Learning:** Developers often use `console.log` for debugging and forget to remove them before production, leading to unintentional information leakage of user activity data.
