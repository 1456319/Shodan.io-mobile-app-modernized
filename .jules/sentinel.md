...
**Vulnerability:** The application was logging sensitive user search history (`this.searches`) and bookmarks (`this.bookmarks`) to the browser console in `src/app/history/history.page.ts` and `src/app/my-searches/my-searches.page.ts`.
**Learning:** Developers often use `console.log` for debugging during development and forget to remove them before production, leading to unintentional information leakage of internal application state and sensitive user data like search queries and saved bookmarks.
