...
**Vulnerability:** The application was logging the user's search query (this.query) and raw search results (res and res['matches']) to the browser console in src/app/search-results/search-results.page.ts.
**Learning:** Developers often use console.log for debugging during development and forget to remove them before production, leading to unintentional information leakage of internal application state and user activity.
**Prevention:** Establish a strict policy against logging sensitive user queries and search results. Use a dedicated logging service that automatically strips or masks sensitive information before writing to logs.
