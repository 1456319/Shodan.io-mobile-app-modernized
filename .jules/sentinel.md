...
## 2026-07-19 - [MEDIUM] Fix Sensitive Data Leak in Console Logs
**Vulnerability:** The application was logging potentially sensitive data (network alert info, Shodan queries, host details, and port payloads) directly to the browser console in `src/app/alerts-details/alerts-details.page.ts`, `src/app/home/home.page.ts`, `src/app/host-results/host-results.page.ts`, and `src/app/queries/queries.page.ts`.
**Learning:** Developers often use `console.log` for debugging during development and forget to remove them before production, leading to unintentional information leakage of internal application state, user queries, and system details.
**Prevention:** Establish a strict policy against logging sensitive data. Use a dedicated logging service that automatically strips or masks sensitive information before writing to logs, and ensure console logs are stripped during the production build process.
