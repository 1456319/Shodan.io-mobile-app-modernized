...
## 2026-05-28 - [CRITICAL] Fix Sensitive Data Leak in Console Logs
**Vulnerability:** The application was logging sensitive user search history, queries, and search results to the browser console (`console.log(this.searches);`, `console.log(this.query);`, and `console.log(res);`) in `src/app/history/history.page.ts` and `src/app/search-results/search-results.page.ts`.
**Learning:** Developers often use `console.log` for debugging during development and forget to remove them before production, leading to unintentional information leakage of internal application state, user history, and query results.
**Prevention:** Establish a strict policy against logging sensitive data or generic error objects. Use a dedicated logging service that automatically strips or masks sensitive information before writing to logs or error tracking systems.
