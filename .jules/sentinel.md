...
**Vulnerability:** The application was logging the user's entire search history array (`this.searches`) to the browser console (`console.log(this.searches);`) in `src/app/history/history.page.ts`.
**Learning:** Even when debugging specific features like search history, logging user-specific data can lead to information leakage and violate privacy expectations.
**Prevention:** Strictly enforce a "no console.log" policy for production code, especially when dealing with data retrieved from storage services that might contain sensitive user history.
