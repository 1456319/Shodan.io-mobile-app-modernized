...
**Vulnerability:** The application was logging sensitive user search history (`searches`) to the browser console (`console.log(this.searches);`) in `src/app/history/history.page.ts`.
**Learning:** Even when debugging specific features like search history, logging data structures to the console can lead to significant information leakage of user-specific and potentially sensitive data.
**Prevention:** Strictly enforce a "no console.log" policy for production code, especially when dealing with data retrieved from storage services. Use safe logging abstractions that sanitize data.
